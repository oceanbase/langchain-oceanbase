...
class TestOceanbaseVectorStoreIntegration:
    ...
    def test_basic_add_and_search(self, vectorstore):
        ...
        # Test similarity search with score - should return some results
        results_with_score = vectorstore.similarity_search_with_score("test query", k=2)
        assert len(results_with_score) >= 1  # Should return at least one result
        assert all(
            isinstance(result, tuple) and len(result) == 2
            for result in results_with_score
        )

        # Test that we can retrieve documents by ID
    ...
    def test_different_metric_types(self, vectorstore):
        ...
        for metric_type in metric_types:
            ...
            documents = [
                Document(
                    page_content=f"Test for {metric_type}",
            ...
            # Test with empty query to get all documents
    ...
    def test_different_index_types(self, vectorstore):
        ...
        for index_type in index_types:
            ...
            documents = [
                Document(
                    page_content=f"Test for {index_type} index",
    ...
    def test_metadata_preservation(self, vectorstore):
        ...
        documents = [
            Document(
                page_content="Integration metadata content 1",
                metadata={"key1": "value1", "key2": "value2"},
            ),
            Document(
                page_content="Integration metadata content 2",
